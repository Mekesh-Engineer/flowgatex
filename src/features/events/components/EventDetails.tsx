...
interface EventDetailsProps {
  event: (CreateEventData & { id: string }) | null | undefined;
  isLoading?: boolean;
}
...
export default function EventDetails({ event, isLoading }: EventDetailsProps) {
  const navigate = useNavigate();
  
  // Fetch all events for "You Might Also Like"
  ...
  const serviceFee = quantity * 12; // Standard service fee
  ...
  const handleBook = () => {
    ...
    navigate(`/booking/checkout?eventId=${event.id}&tierId=${selectedTierId}&qty=${quantity}`);
  };
    ...
    return (
      <div className="w-full min-h-screen bg-[var(--bg-base)] animate-pulse p-4 lg:p-20 flex flex-col items-center">
        <div className="max-w-7xl w-full flex flex-col gap-8">
           <div className="h-[500px] bg-[var(--bg-card)] rounded-3xl w-full border border-[var(--border-primary)]" />
  ...
  return (
    <div className="bg-[var(--bg-base)] min-h-screen flex flex-col font-sans text-[var(--text-primary)] transition-colors duration-300 relative overflow-x-hidden">
      ...
      <main className="relative z-10 flex-grow w-full flex justify-center px-4 md:px-8 lg:px-12 py-8 lg:py-16">
        <div className="max-w-[1400px] w-full flex flex-col gap-12 lg:gap-16">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16">
            ...
            <div className="lg:col-span-8 flex flex-col gap-12">
              ...
              >
                ...
                <div className="rounded-[2rem] border border-[var(--border-primary)] bg-[var(--bg-card)] p-8 shadow-sm">
                  <div 
              ...
              >
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {amenities.map((amenity, idx) => (
                    <div key={idx} className={`flex flex-col items-center justify-center gap-3 p-6 rounded-[1.5rem] border transition-all duration-300 ${amenity.active ? 'border-[var(--border-primary)] bg-[var(--bg-card)] hover:border-[var(--color-primary)] hover:shadow-md' : 'border-dashed border-[var(--border-primary)] bg-[var(--bg-base)] opacity-50 grayscale'}`}>
                ...
                >
                  ...
                  <div className="space-y-6">
                    {agendaGroups && agendaTabs.find(t => t.id === activeTab) && agendaGroups[agendaTabs.find(t => t.id === activeTab)!.originalKey]?.map((item, idx) => (
                      <div 
                         key={idx} 
                         className="flex flex-col md:flex-row gap-6 p-6 rounded-[2rem] bg-[var(--bg-card)] border border-[var(--border-primary)] hover:shadow-lg transition-all duration-300"
                      >
                         <div className="flex-shrink-0 w-full md:w-32 flex flex-col items-center justify-center p-4 bg-[var(--bg-surface)] rounded-2xl border border-[var(--border-primary)]">
                       ...
                       <div className="text-center py-12 text-[var(--text-muted)] italic bg-[var(--bg-card)] rounded-[2rem] border border-[var(--border-primary)]">
              ...
              >
                ...
                <div className="rounded-[2.5rem] overflow-hidden border border-[var(--border-primary)] bg-[var(--bg-card)] h-[450px] relative shadow-lg group">
                    ...
                    <div className="absolute bottom-6 left-6 z-10 w-[calc(100%-3rem)] md:w-auto">
                        <FloatingElement duration={4}>
                           <div className="bg-[var(--bg-card)] p-6 rounded-2xl border border-[var(--border-primary)] max-w-sm shadow-xl">
                               ...
                               <a 
                                   href={`https://maps.google.com/?q=${event.venue?.mapCoordinates ? `${event.venue.mapCoordinates.lat},${event.venue.mapCoordinates.lng}` : encodeURIComponent(event.venue?.address || '')}`} 
                                   target="_blank" 
                                   rel="noreferrer"
              ...
              <div className="sticky top-24 space-y-6">
                ...
                >
                  ...
                  <div className="flex gap-2 mb-8 bg-[var(--bg-base)] p-1 rounded-2xl border border-[var(--border-primary)]">
                    <input 
                      className="flex-grow bg-transparent border-none px-4 py-3 text-sm text-[var(--text-primary)] focus:ring-0 outline-none placeholder-[var(--text-muted)] font-medium" 
                      placeholder="Have a promo code?" 
                      type="text" 
                      value={promoCode}
                      onChange={(e) => setPromoCode(e.target.value)}
                    />
                    <button className="px-6 py-2 bg-[var(--bg-card)] border border-[var(--border-primary)] text-[var(--text-primary)] text-sm font-bold rounded-xl hover:bg-[var(--text-primary)] hover:text-[var(--bg-base)] transition-all shadow-sm">
                      Apply
                    </button>
                  </div>
                ...
                {/* Share Card (Solid) */}
                <div className="rounded-[2rem] border border-[var(--border-primary)] bg-[var(--bg-card)] p-6 flex items-center justify-between shadow-md">
          ...
          <div className="py-20 border-t border-[var(--border-primary)]">
            ...
            {relatedEvents.length > 0 ? (
                    ...
                    return (
                       <GlowingBorder key={evt.id} className="h-full">
                         <div 
                           ...
                           className="group block rounded-[2rem] overflow-hidden bg-[var(--bg-card)] border border-[var(--border-primary)] hover:shadow-xl transition-all duration-300 cursor-pointer h-full flex flex-col"
                         >
               ...
               <div className="text-center py-12 bg-[var(--bg-card)] rounded-[2rem] border border-[var(--border-primary)]">
