...
import { getAuth, GoogleAuthProvider, FacebookAuthProvider, Auth, connectAuthEmulator } from 'firebase/auth';
import { initializeFirestore, Firestore, persistentLocalCache, persistentMultipleTabManager, getFirestore, connectFirestoreEmulator } from 'firebase/firestore';
...
const EMULATE_FUNCTIONS_ONLY = import.meta.env.VITE_EMULATE_FUNCTIONS_ONLY === 'true';
...
if (firebaseEnabled && app) {
  try {
    auth = getAuth(app);
    if (USE_EMULATORS && !EMULATE_FUNCTIONS_ONLY) {
      connectAuthEmulator(auth, 'http://127.0.0.1:9099');
      logger.log('🔥 Using Auth Emulator');
    }
  } catch (error) {
  ...
  if (firebaseEnabled) {
    ...
    try {
      ...
      if (USE_EMULATORS && !EMULATE_FUNCTIONS_ONLY) {
        ...
        logger.log('🔥 Using Realtime Database Emulator');
      }
    } catch (error) {
    ...
    try {
      ...
      if (USE_EMULATORS && !EMULATE_FUNCTIONS_ONLY) {
        ...
        logger.log('🔥 Using Storage Emulator');
      }
    } catch (error) {
    ...
    try {
      ...
      if (USE_EMULATORS) {
        ...
        logger.log('🔥 Using Functions Emulator');
      }
    } catch (error) {
      logger.warn('⚠️ Cloud Functions initialization failed:', error);
    }
...
export { auth, db, realtimeDb, storage, appCheck, functions };
...
// Helper to get Cloud Functions with runtime null check
export const getFunctionsInstance = (): Functions => {
  if (!functions) {
    throw new Error('Firebase Functions is not initialized. Make sure Firebase is properly configured.');
  }
  return functions;
};
